...
for running end-to-end tests on Shido.
It is used for testing chain upgrades,
as it allows for initializing multiple Shido chains with different versions.
...
# version(s) of initial shido node(s) that will be upgraded, tag e.g. 'v9.1.0'
# to use multiple upgrades separate the versions with a forward slash, e.g.
# 'v10.0.1/v11.0.0-rc1'
INITIAL_VERSION

# version of upgraded shido node that will replace the initial node, tag e.g.
# 'v10.0.0'
TARGET_VERSION

# mount point for the upgraded node container, to mount new node version to
# previous node state folder. By default this is './build/.shidod:/root/.shidod'
# More info at https://docs.docker.com/engine/reference/builder/#volume
MOUNT_PATH

# '--chain-id' shido cli parameter, used to start nodes with a specific
# chain-id and submit proposals
# By default this is 'shido_9000-1'
CHAIN_ID
```
...
1. Build a docker image for the shido target version
...
the test suite exports `/.shidod` folder from the docker container
...
This package is decoupled from depending on the Shido codebase.
It initializes the chains for testing via Docker.  
As a result, the test suite may provide the
desired Shido version to Docker containers during the initialization.
This design allows for the opportunity of testing chain upgrades
by providing an older Shido version to the container,
...
to run different versions of shido containers,
...
will be pulled from [DockerHub](https://hub.docker.com/r/tharsishq/shido/tags).
If it is not specified,
the test suite retrieves the second-to-last upgrade version
from the local codebase (in the `shido/app/upgrades` folder)
...
```log
ok  	github.com/shido/shido/v9/tests/e2e	174.137s.
```
...
Error:  Received unexpected error:
        can't start shido node, container exit code: 2
        ...
        main.appCreator.newApp({{{0x3399b40, 0xc000ec1db8}, {0x33ac0f8, 0xc00...
            github.com/shido/shido/v10/cmd/shidod/root.go:243 +0x2ca
        github.com/shido/ethermint/server.startInProcess(_, {{0x0, 0x0, 0x0},...
            github.com/shido/ethermint@v0.20.0-rc2/server/start.go:304 +0x9c5
        github.com/shido/ethermint/server.StartCmd.func2(0xc001620600?, {0xc0...
            github.com/shido/ethermint@v0.20.0-rc2/server/start.go:123 +0x1ec
        ...
        main.main()
            github.com/shido/shido/v10/cmd/shidod/main.go:20 +0x45
...
```log
CONTAINER ID   IMAGE
9307f5485323   shido:local    <-- upgraded node
f41c97d6ca21   shido:v9.0.0   <-- initial node
```
