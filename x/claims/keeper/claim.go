...
func (k Keeper) MergeClaimsRecords(
	...
	for _, action := range actions {
		...
		//  - the sender is not an shido address and can't claim vote, delegation or evm actions
		...
		if senderClaimsRecord.HasClaimedAction(action) {
			return types.ClaimsRecord{}, errorsmod.Wrapf(errortypes.ErrNotSupported, "non-shido sender must not have claimed action: %v", action)
		}
