...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	shidoMeta := banktypes.Metadata{
		Description: "Base Denom for Shido Chain",
		Base:        utils.BaseDenom,
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.AshidoDenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    utils.BaseDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.AshidoDenomtrace.BaseDenom,
	}
	...
	Describe("disabled params", func() {
		BeforeEach(func() {
			...
			err := s.app.Erc20Keeper.SetParams(s.ShidoChain.GetContext(), erc20params)
		...
		It("should transfer and not convert to erc20", func() {
			...
			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.ShidoChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			...
			ibcOsmoBalanceAfter := s.app.BankKeeper.GetBalance(s.ShidoChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(amount, ibcOsmoBalanceAfter.Amount.Int64())

			// check ERC20 balance - should be zero (no conversion)
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and convert uosmo to tokens", func() {
			...
			balanceTokenBefore := s.app.Erc20Keeper.BalanceOf(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			s.Require().Equal(int64(0), balanceTokenBefore.Int64())

			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.ShidoChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcOsmoBalanceBefore.Amount.Int64())

			s.ShidoChain.Coordinator.CommitBlock()
			// Send coins
			s.SendAndReceiveMessage(s.pathOsmosisShido, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")
		...
		It("should transfer and not convert unregistered coin (uatom)", func() {
			...
			// check balance before transfer is 0
			ibcAtomBalanceBefore := s.app.BankKeeper.GetBalance(s.ShidoChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
			s.Require().Equal(int64(0), ibcAtomBalanceBefore.Amount.Int64())

			s.ShidoChain.Coordinator.CommitBlock()
			s.SendAndReceiveMessage(s.pathCosmosShido, s.IBCCosmosChain, "uatom", amount, sender, receiver, 1, "")
		...
		It("should transfer and convert original erc20", func() {
			...
			// 1. Send 'uosmo' from Osmosis to Shido
			s.SendAndReceiveMessage(s.pathOsmosisShido, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// validate 'uosmo' was transferred successfully and converted to ERC20
			balanceERC20Token := s.app.Erc20Keeper.BalanceOf(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			s.Require().Equal(amount, balanceERC20Token.Int64())

			// 2. Transfer back the erc20 from Shido to Osmosis
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.UosmoDenomtrace.Path, teststypes.UosmoDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisShido, s.ShidoChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount, receiver, sender, 1, ibcCoinMeta)
	...
	Describe("Performing recovery with registered coin", func() {
		...
		It("should recover and not convert uosmo to tokens", func() {
			...
			// Send 'uosmo' to Osmosis address in Shido Chain (locked funds)
			// sender_addr == receiver_addr
			s.SendAndReceiveMessage(s.pathOsmosisShido, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")
			timeout := uint64(s.ShidoChain.GetContext().BlockTime().Add(time.Hour * 4).Add(time.Second * -20).UnixNano())
			err := s.pathOsmosisShido.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
			s.Require().NoError(err)
	...
	Describe("registered erc20", func() {
		BeforeEach(func() { //nolint:dupl
			...
			sender = s.ShidoChain.SenderAccount.GetAddress().String()
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
			senderAcc = sdk.MustAccAddressFromBech32(sender)
			...
			s.Require().NoError(err)
			pair, err = s.app.Erc20Keeper.RegisterERC20(s.ShidoChain.GetContext(), addr)
			s.Require().NoError(err)
			...
			s.ShidoChain.SenderAccount.SetSequence(s.ShidoChain.SenderAccount.GetSequence() + 1) //nolint:errcheck
		})
		It("should convert erc20 ibc voucher to original erc20", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			...
			_, err = s.app.Erc20Keeper.ConvertERC20(sdk.WrapSDKContext(s.ShidoChain.GetContext()), msgConvertERC20)
			s.Require().NoError(err)
			...
			erc20CoinsBalance := s.app.BankKeeper.GetBalance(s.ShidoChain.GetContext(), senderAcc, pair.Denom)
			s.Require().Equal(amount, erc20CoinsBalance.Amount.Int64())

			s.ShidoChain.Coordinator.CommitBlock()

			// Attempt to send erc20 into ibc, should send without conversion
			s.SendBackCoins(s.pathOsmosisShido, s.ShidoChain, pair.Denom, amount, sender, receiver, 1, pair.Denom)
			...
			s.SendAndReceiveMessage(s.pathOsmosisShido, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
		})
		It("should convert full available balance of erc20 coin to original erc20 token", func() {
			...
			// Attempt to send erc20 into ibc, should send without conversion
			s.SendBackCoins(s.pathOsmosisShido, s.ShidoChain, pair.Denom, amount/2, sender, receiver, 1, pair.Denom)
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			...
			s.SendAndReceiveMessage(s.pathOsmosisShido, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount/2, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
		...
		It("send native ERC-20 to osmosis, when sending back IBC coins should convert full balance back to erc20 token", func() {
			...
			s.SendBackCoins(s.pathOsmosisShido, s.ShidoChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount/2, sender, receiver, 1, "")
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			...
			// send back the IBC coins from Osmosis to Shido
			s.SendAndReceiveMessage(s.pathOsmosisShido, s.IBCOsmosisChain, erc20Denomtrace.IBCDenom(), amount/2, receiver, sender, 1, erc20Denomtrace.GetFullDenomPath())
			// Check Balance
			balanceToken = s.app.Erc20Keeper.BalanceOf(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount, balanceToken.Int64())
...
var _ = Describe("Convert outgoing ERC20 to IBC", Ordered, func() {
	...
	Describe("disabled params", func() {
		...
		It("should fail transfer and not convert to IBC", func() {
			...
			denom := originChain.App.(*app.Shido).StakingKeeper.BondDenom(originChain.GetContext())
	...
	Describe("registered erc20", func() {
		...
		It("should transfer available balance", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount*2))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount*2, balanceToken.Int64())
		...
		It("should timeout and reconvert to ERC20", func() {
			...
			originChain := s.ShidoChain
			coin := pair.Denom
			currentTime := s.ShidoChain.Coordinator.CurrentTime
			...
			// need to update shido chain to prove missing ack
	...
	Describe("registered coin", func() {
		BeforeEach(func() {
			...
			// Send from osmosis to Shido
			s.SendAndReceiveMessage(s.pathOsmosisShido, s.IBCOsmosisChain, "uosmo", amount, receiver, sender, 1, "")
			s.ShidoChain.Coordinator.CommitBlock(s.ShidoChain)
			erc20params.EnableErc20 = true
			err = s.app.Erc20Keeper.SetParams(s.ShidoChain.GetContext(), erc20params)
			s.Require().NoError(err)
		...
		It("should convert erc20 to ibc vouched and transfer", func() {
			...
			balance := s.app.BankKeeper.GetBalance(s.ShidoChain.GetContext(), senderAcc, teststypes.UosmoIbcdenom)
			...
			s.Require().NoError(err)
			// Use MsgConvertERC20 to convert the ERC20 to a Cosmos IBC Coin
			_, err = s.app.Erc20Keeper.ConvertCoin(sdk.WrapSDKContext(s.ShidoChain.GetContext()), msgConvertCoin)
			s.Require().NoError(err)

			s.ShidoChain.Coordinator.CommitBlock()

			// Attempt to send erc20 tokens to osmosis and convert automatically
			s.SendBackCoins(s.pathOsmosisShido, s.ShidoChain, pair.Denom, amount, sender, receiver, 1, teststypes.UosmoDenomtrace.GetFullDenomPath())
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			// Check balance on the Osmosis chain
		...
		It("should timeout and reconvert coins", func() {
			...
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.ShidoChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(int64(0), balanceERC20TokenAfter.Int64())
