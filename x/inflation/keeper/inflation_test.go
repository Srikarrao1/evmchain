...
func (suite *KeeperTestSuite) TestGetCirculatingSupplyAndInflationRate() {
	...
	bondedCoins := sdk.NewDecCoin(shidotypes.AttoShido, bondedAmt)
	...
	testCases := []struct {
	...
	}{
		{
			...
			sdk.TokensFromConsensusPower(400_000_000, shidotypes.PowerReduction).Sub(bondedAmt),
			func() {
		...
		{
			"high supply",
			sdk.TokensFromConsensusPower(800_000_000, shidotypes.PowerReduction).Sub(bondedAmt),
			func() {},
		...
		},
		{
			...
			sdk.TokensFromConsensusPower(200_000_000, shidotypes.PowerReduction).Sub(bondedAmt),
			func() {},
			sdk.ZeroDec(),
		},
	}
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.ctx = suite.ctx.WithChainID("shido_9001-1")
			tc.malleate()
			...
			teamAlloc := sdk.NewDecCoin(
				types.DefaultInflationDenom,
				sdk.TokensFromConsensusPower(int64(200_000_000), shidotypes.PowerReduction),
			)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.isMainnet {
				...
				suite.ctx = suite.ctx.WithChainID("shido_9999-666")
			}
			tc.malleate()
