...
import (
	...
	"github.com/shido/shido/v2/x/recovery/keeper"
...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	coins := sdk.NewCoins(
		sdk.NewCoin("ashido", sdk.NewInt(1000)),
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrShido, ethsecpAddrCosmos, "")
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", "shido", ethsecpAddrCosmos, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrShido, "badbadhf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625", "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, shidoChannel, timeoutHeight, 0)
			},
			false,
		...
		},
		{
			...
			func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, otherSecpAddrShido, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, shidoChannel, timeoutHeight, 0)
			},
			true,
		...
		},
		{
			...
			func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrCosmos, ethsecpAddrShido, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, shidoChannel, timeoutHeight, 0)
			},
			true,
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrShido, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, shidoChannel, timeoutHeight, 0)
		...
		{
			"recovery - send uatom from cosmos to shido",
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrShido, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, shidoChannel, timeoutHeight, 0)
		...
		{
			"recovery - send ibc/uosmo from cosmos to shido",
			func() {
				denom = ibcOsmoDenom
		...
		{
			"recovery - send uosmo from osmosis to shido",
			func() {
				// Setup Osmosis <=> Shido IBC relayer
				denom = "uosmo"
				sourceChannel = "channel-204"
				shidoChannel = claimstypes.DefaultAuthorizedChannels[0]
				path = fmt.Sprintf("%s/%s", transfertypes.PortID, shidoChannel)
...
func (suite *KeeperTestSuite) TestOnRecvPacketFailTransfer() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrShido, "")
			packet := channeltypes.NewPacket(transfer.GetBytes(), 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, shidoChannel, timeoutHeight, 0)
			...
			// Fund receiver account with SHIDO
			coins := sdk.NewCoins(
				sdk.NewCoin("ashido", sdk.NewInt(1000)),
				sdk.NewCoin(ibcAtomDenom, sdk.NewInt(1000)),
			)
